...
function findById(id) {
  return films.find((f) => f.imdbID === id);
}
...
function findIndexByTitle(title) {
  return films.findIndex((f) => f.Title === title);
}
...
function addShortSummary(film) {
  return {
    ...film,
    shortSummary: `${film.Title}-${film.Year}-${film.Released}`,
  };
}
...
function getAllDirectors() {
  ...
  films.forEach((f) => (f.Director !== "N/A" ? allDir.push(f.Director) : []));
  return allDir;
}
...
function filterByType(type) {
  return films.filter((f) => (f.Type === type ? true : false));
}
...
function filterByGenre(genres) {
  return films.filter((f) => {
    let isInGenre = false;
    genres.forEach((g) => {
      f.Genre.includes(g) ? (isInGenre = true) : null;
    });
    return isInGenre;
  });
}
...
function getActorsList() {
  return films.map((f) => f.Actors.split(", ")).flat();
}
...
function filmsUnderAnHour() {
  let underHour = [];
  films.filter((f) =>
    parseInt(f.Runtime) < 60 ? underHour.push(f.Title) : []
  );
  return underHour;
}
...
function filterByImdbRating(targetRating) {
  return films.filter((f) => {
    if (targetRating !== "N/A") {
      return parseFloat(f.imdbRating) >= targetRating;
    } else if (targetRating === "N/A") {
      return f.imdbRating === "N/A";
    }
  });
}
...
// console.log(filterByImdbRating("N/A").length); //4
...
function getMovieProperties(properties) {
  return films.map((f) => {
    ...
    properties.forEach((p) => (mappedFilm[p] = f[p]));
    return mappedFilm;
  });
}
...
function getMovieAwards() {
  let filtered = films.filter((f) => f.Awards !== "N/A");

  let mappedAwards = filtered.map((m) => {
    return `${m.Title}: ${m.Awards}`;
  });
  return mappedAwards;
}
